...
#Region "Methods - Public"
    ...
    ''' </summary>
    ''' <param name="keyShort">Parameter short name, e.g. 'f' for /f</param>
    ''' <param name="keyLong">Parameter long name, e.g. 'file' for /file</param>
    ''' <returns>
    ''' <see langword="true"/> if parameter is present, otherwise <see langword="false"/>
    ''' </returns>
    Public Overloads Function HasParameter(ByVal keyShort As String, ByVal keyLong As String) As Boolean

      With Me
        Return (.HasParameter(keyShort) OrElse .HasParameter(keyLong))
      End With
    ...
    ''' Return the value of a parameter
    ''' </summary>
    ''' <param name="keyShort">The parameter's short name (<see cref="KeyValue.Key"/></param>
    ''' <param name="keyLong">The parameter's long name (<see cref="KeyValue.Key"/></param>
    ''' <param name="caseSensitive">Treat the name as case-sensitive?</param>
    ''' <returns>Returns the <see cref="KeyValue.Value"/> of a parameter by its (parameter) name.</returns>
    Public Function GetValueByName(ByVal keyShort As String, ByVal keyLong As String, Optional ByVal caseSensitive As Boolean = False) As Object

      ' Safe guard
      If HasParameter(keyShort, keyLong) = False Then
        Throw New ArgumentException("Parameter doesn't exist: " & keyShort & ", " & keyLong)
      End If

      For Each o As KeyValue In Me.KeyValues
        If caseSensitive = False Then
          If o.Key.ToLower = keyShort.ToLower Then
            Return o.Value
          End If
        Else
          If o.Key = keyShort Then
            Return o.Value
          End If
        End If
      Next
      ...
      For Each o As KeyValue In Me.KeyValues
        If caseSensitive = False Then
          If o.Key.ToLower = keyLong.ToLower Then
            Return o.Value
          End If
        Else
          If o.Key = keyLong Then
            Return o.Value
          End If
        End If
      Next
...
#Region "Constructor/Dispose"
    ...
    ''' </summary>
    ''' <param name="keyValueList">Command line parameters as a List(Of <see cref="KeyValue"/></param>
    Public Sub New(ByVal keyValueList As List(Of KeyValue))
